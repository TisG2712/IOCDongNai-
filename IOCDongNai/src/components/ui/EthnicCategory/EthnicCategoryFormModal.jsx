...
function EthnicCategoryFormModal({ open, onClose, onSave, editingCategory }) {
  ...
  useEffect(() => {
    if (editingCategory) {
      setForm({
        ...editingCategory,
        kichHoat:
          editingCategory.kichHoat !== undefined
            ? editingCategory.kichHoat
            : true,
      });
    } else {
      setForm({
          ...
          onSubmit={(e) => {
            ...
            onSave({
              ...
              locked:
                form.kichHoat === false
                  ? true
                  : editingCategory?.locked || false,
            });
          }}
        >
          <div>
            <label className="block mb-1 font-medium">Tên dân tộc</label>
            <input
...
export default EthnicCategoryFormModal;
