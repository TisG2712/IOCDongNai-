...
function SystemParameterFormModal({ open, onClose, onSave, editingParameter }) {
  ...
  useEffect(() => {
    if (editingParameter)
      setForm({
        ...editingParameter,
        kichHoat:
          editingParameter.kichHoat !== undefined
            ? editingParameter.kichHoat
            : true,
      });
    else
      setForm({
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-xs">
      <div className="bg-white p-4 rounded shadow-lg min-w-[260px] max-w-[340px] w-full relative text-xs">
        ...
        <h2 className="text-base font-bold mb-3 text-center">
          {editingParameter ? "Chỉnh sửa" : "Thêm mới"}
        </h2>
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            onSave({
              ...form,
              id: editingParameter?.id || Date.now(),
            });
          }}
        >
          <div>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            >
              {editingParameter ? "Cập nhật" : "Thêm mới"}
            </button>
          </div>
        </form>
      </div>
...
export default SystemParameterFormModal;
