...
function NotificationFormModal({ open, onClose, onSave, editingNotification }) {
  ...
  useEffect(() => {
    if (editingNotification) {
      setForm({
        ...editingNotification,
        kichHoat:
          editingNotification.kichHoat !== undefined
            ? editingNotification.kichHoat
            : true,
      });
    } else {
      setForm({
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-xs">
      <div className="bg-white p-4 rounded shadow-lg min-w-[260px] max-w-[340px] w-full relative text-xs">
        ...
        <h2 className="text-base font-bold mb-3 text-center">
          {editingNotification ? "Chỉnh sửa" : "Thêm mới"}
        </h2>
        <form
          className="space-y-3"
          onSubmit={(e) => {
            ...
            onSave({
              ...
              locked:
                form.kichHoat === false
                  ? true
                  : editingNotification?.locked || false,
            });
          }}
        >
          <div>
            <label className="block mb-1 font-medium">Tên thông báo</label>
            <input
