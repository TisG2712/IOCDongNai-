...
function OfficerFormModal({ open, onClose, onSave, editingOfficer }) {
  ...
  useEffect(() => {
    if (editingOfficer)
      setForm({
        ...editingOfficer,
        kichHoat:
          editingOfficer.kichHoat !== undefined
            ? editingOfficer.kichHoat
            : true,
      });
    else
      setForm({
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-xs">
      <div className="bg-white p-4 rounded shadow-lg min-w-[260px] max-w-[340px] w-full relative text-xs">
        ...
        <h2 className="text-base font-bold mb-3 text-center">
          {editingOfficer ? "Chỉnh sửa" : "Thêm mới"}
        </h2>
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            onSave({
              ...form,
              id: editingOfficer?.id || Date.now(),
              locked:
                form.kichHoat === false
                  ? true
                  : editingOfficer?.locked || false,
            });
          }}
        >
          {/* Tên nhóm */}
