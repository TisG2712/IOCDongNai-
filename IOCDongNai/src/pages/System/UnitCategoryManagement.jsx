...
import UnitCategoryTable from "../../components/ui/UnitCategory/UnitCategoryTable";
import UnitCategoryFormModal from "../../components/ui/UnitCategory/UnitCategoryFormModal";
import ConfirmModal from "../../components/ui/UserGroups/ConfirmModal"; // có thể tái dùng confirm modal cũ
import PageNavigation from "../../components/ui/PageNavigation";
import Search from "../../components/ui/Search";
import { FaPlus } from "react-icons/fa";
import { useSidebar } from "../../components/ui/SidebarContext";

function UnitCategoryManagement() {
  const { isOpen } = useSidebar();

  // Đọc dữ liệu từ localStorage khi khởi tạo
  const [unitCategories, setUnitCategories] = useState(() => {
    const saved = localStorage.getItem("unitCategories");
  ...
  const filteredCategories = unitCategories.filter((c) => {
  ...
  useEffect(() => {
    localStorage.setItem("unitCategories", JSON.stringify(unitCategories));
  }, [unitCategories]);
  ...
  const handleSave = (category) => {
    ...
    setCurrentPage(1); // về trang đầu khi thêm mới
  };
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message="Bạn có chắc chắn muốn xóa nhóm đơn vị hành chính này không?"
      />
      <Footer />
    </>
  );
...
export default UnitCategoryManagement;
